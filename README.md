...
- 历史查询默认 24 小时，可选 1/24/168/720 小时；响应上限为 2 MiB，并通过复合时间索引、每成员地图上限和持久限流控制成本。
- 新家庭组号和默认邀请码均为 8 位小写字母数字；旧 32 位十六进制组号继续作为唯一别名兼容读取，自定义邀请码支持 4 至 64 位字母数字。
- 前台原生 App 可连接已认证的 `/api/events` WebSocket；事件按家庭组隔离，带心跳、连接/订阅上限和慢消费者保护，断线时仍可回退 REST 刷新。

## 公开隐私边界

登录后的私有历史会保留 IP/WebRTC 探测来源中可用的精确地址、坐标和候选证据，供地图下方的详细数据与历史位置文本展示；地图本身只使用 GPS 坐标与 GPS 地址。公开分享由服务端重新投影到严格白名单，只保留成员显示名、时间、公开地图坐标、城市、地址和必要的坐标系信息；`sources`、`variants`、`candidates`、IP 地址、WebRTC/STUN 与其他网络诊断字段不会进入公开分享响应，纯 IP 文本也不会作为地址回退公开。
...
数据库首次启动使用 `schema_core.sql` 建立基础表，后续结构变化按 `internal/database/migrations/` 中的版本文件顺序执行并记录到 `schema_migrations`；历史时间窗使用迁移 `012` 的复合索引，迁移 `013` 禁止未绑定家庭组的邀请码继续兑换。
...
- `deploy/nginx-go-backend.sample.conf`：Nginx `/api/events` WSS upgrade、普通 `/api/` 纯 Go反代及禁用旧网页后台的样例。
