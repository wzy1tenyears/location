...
用户端“我的”页提供可选的“无障碍保活”入口。只有用户点击并在 Android 系统无障碍页面手动确认后才会启用，App 不会在启动时自动申请或静默开启。

该服务只监听本应用的窗口状态以维持系统绑定，事件回调不读取或保存内容；配置明确禁止读取窗口内容、查看其他应用、执行手势、截图、触摸探索和按键过滤。用户可随时回到系统无障碍页面关闭。
...
地图只显示 GPS 坐标标记，不绘制 IP 或 WebRTC 位置。地图下方的详细数据与历史位置文本仍保留 IP/WebRTC 探测结果，以及区县、街道、详情、POI、邮编和坐标系等结构化字段。
