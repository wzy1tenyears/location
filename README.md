...
一个基于 PHP + MySQL + 原生 Android 客户端的家庭定位共享项目。服务端部署在 PHP/Nginx/MySQL 环境中，Android 客户端为原生实现，不使用 WebView。
...
至少需要确认数据库、Redis、后台账号、后台路径、地图 Key、IP 探测 Token 和 Cloudflare Turnstile 配置。

`private/config.php` 含敏感信息，线上 Nginx 必须禁止外部访问 `/private/`。

## Android 客户端

公开版保留用户端 Android 源码，不提供 APK、签名文件或私有后台客户端。打包前请写入服务端 URL：
...
- `assets/`：网页端资源。
- `admin/`：后台管理源码目录，实际路径由 `private/config.php` 控制。
- `private/`：配置、公共库和安装 SQL，必须禁止公网访问。
