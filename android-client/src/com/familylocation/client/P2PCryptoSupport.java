...
final class P2PCryptoSupport {
    ...
    private static final class GroupKeyMaterial {
        final int keyVersion;
        final byte[] rawGroupKey;

        GroupKeyMaterial(int keyVersion, byte[] rawGroupKey) {
            this.keyVersion = keyVersion;
            this.rawGroupKey = rawGroupKey;
        }
    }
    ...
    static JSONObject status(JsonTransport transport, String groupName) throws Exception {
        return transport.postJson(ApiPaths.P2P_CRYPTO, new JSONObject()
            .put("action", "status")
            .put("group_name", groupName));
    }

    static JSONObject publishKey(JsonTransport transport, Context context, String groupName, String currentPassword) throws Exception {
        ...
        return transport.postJson(ApiPaths.P2P_CRYPTO, payload);
    }

    static JSONObject setConsent(JsonTransport transport, Context context, String groupName, String currentPassword, boolean consent) throws Exception {
        publishKey(transport, context, groupName, currentPassword);
        return transport.postJson(ApiPaths.P2P_CRYPTO, new JSONObject()
            .put("action", "consent")
            .put("group_name", groupName)
            .put("consent", consent));
    ...
    static JSONObject decryptRecord(JsonTransport transport, Context context, String fallbackGroupName, JSONObject record) {
        ...
        decryptRecords(transport, context, fallbackGroupName, records);
        return records.optJSONObject(0);
    }

    static void decryptRecords(JsonTransport transport, Context context, String fallbackGroupName, JSONArray records) {
        if (records == null || records.length() == 0) {
            return;
        }
        Map<String, GroupKeyMaterial> materials = new LinkedHashMap<>();
        Map<String, Boolean> unavailableGroups = new LinkedHashMap<>();
        try {
            for (int index = 0; index < records.length(); index += 1) {
                ...
                String groupName = recordGroupName(record, fallbackGroupName);
                if (groupName.isEmpty() || materials.containsKey(groupName) || unavailableGroups.containsKey(groupName)) {
                    continue;
                }
                try {
                    JSONObject current = status(transport, groupName);
                    ...
                    materials.put(groupName, new GroupKeyMaterial(
                        current.optInt("key_version", 0),
                        unwrapGroupKey(context, wrappedKey)
                    ));
                } catch (Exception exception) {
                    unavailableGroups.put(groupName, true);
                }
            }
            for (int index = 0; index < records.length(); index += 1) {
                JSONObject record = records.optJSONObject(index);
                ...
                String groupName = recordGroupName(record, fallbackGroupName);
                GroupKeyMaterial material = materials.get(groupName);
                int recordKeyVersion = record.optInt("p2p_key_version", 0);
                if (material == null
                    || (recordKeyVersion > 0 && material.keyVersion > 0 && recordKeyVersion != material.keyVersion)) {
                    replaceRecord(records, index, unreadableRecord(record));
                    continue;
                }
                replaceRecord(records, index, decryptRecordWithKey(record, material.rawGroupKey));
            }
        } finally {
            for (GroupKeyMaterial material : materials.values()) {
                Arrays.fill(material.rawGroupKey, (byte) 0);
            }
        }
    }

    private static String recordGroupName(JSONObject record, String fallbackGroupName) {
        String groupName = record == null ? "" : record.optString("group_name", "").trim();
        return groupName.isEmpty() ? (fallbackGroupName == null ? "" : fallbackGroupName.trim()) : groupName;
    }

    private static JSONObject decryptRecordWithKey(JSONObject record, byte[] rawGroupKey) {
        try {
            ...
            return merged.put("p2p_decrypted", true);
        } catch (Exception exception) {
            return unreadableRecord(record);
        }
    }

    private static JSONObject unreadableRecord(JSONObject record) {
        try {
    ...
    private static void replaceRecord(JSONArray records, int index, JSONObject record) {
        try {
