...
async function reverseGpsAddress(latitude, longitude) {
    ...
    if (isUsefulAddressResult(meituanResult, fallback)) {
        return meituanResult;
    }

    const amapResult = await reverseGpsByAmap(latitude, longitude, fallback);
    if (isUsefulAddressResult(amapResult, fallback)) {
        return amapResult;
    }

    const bigDataCloudResult = await reverseGpsByBigDataCloud(latitude, longitude, fallback);
    return isUsefulAddressResult(bigDataCloudResult, fallback) ? bigDataCloudResult : fallback;
}
...
async function reverseGpsByMeituan(latitude, longitude, fallback) {
    try {
        const url = `https://apimobile.meituan.com/group/v1/city/latlng/${encodeURIComponent(latitude)},${encodeURIComponent(longitude)}?tag=0`;
        ...
        const address = parts.length ? parts.join('') : fallback.address;
        const city = result.city || result.openCityName || inferCityFromText(address);
